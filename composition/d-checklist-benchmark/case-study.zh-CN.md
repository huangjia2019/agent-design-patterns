...
结论先说。同一份金融产品披露规范，单次生成只命中 12 条标准规则里的 6 条。加入生成评审和自我修正后到 7 条。引入候选队列后到 9 条。把多条路径的结果保留下来形成高召回审阅队列，可以到 10 条。
...
如果只看单个模式，结论可能是"用生成评审就够了"。但一旦进入组合视角，答案变了：先用 baseline 确定难度，再用 candidate-guided review 拉高覆盖，再用 union queue 保留互补候选，最后交给人工审核。
