...
pattern.py 已落地（跟随极客时间《Agent 设计模式之美》第 29 讲 / 06-04）。核心判断：**一条教训能留在回放池里，靠的是复用后的真实成功率，不是 Agent 觉得它有用**。经验分层存（L0 原始轨迹留审计 / L1 单任务教训供注入 / L2 跨任务规律），检索命中后 render 成上层背景层包住当前决策。每次复用把下游成败回写（EMA），复用够多仍低于健康线的自动归档出池。只有显式标记为确定性可判、同时战绩过硬的教训，才会成为动作前护栏候选（接第 25 讲），软教训随之退休。
